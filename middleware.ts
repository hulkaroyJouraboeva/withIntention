...
export async function middleware(request: NextRequest) {
    try {
        ...
        // Fallback for debugging the Vercel 500 Error
        return new NextResponse(
            `Middleware Crash Report:
Error Message: ${error?.message || 'Unknown error'}
Missing URL ? ${!process.env.NEXT_PUBLIC_SUPABASE_URL}
Missing Anon Key ? ${!process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY} `,
            { status: 500 }
        )
    }
}
